...
/* harmony import */ var _vendor_map_api_js__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./vendor/map-api.js */ "./src/js/vendor/map-api.js");
/* harmony import */ var _vendor_map_api_js__WEBPACK_IMPORTED_MODULE_2___default = /*#__PURE__*/__webpack_require__.n(_vendor_map_api_js__WEBPACK_IMPORTED_MODULE_2__);
...
var itemAccord = document.querySelectorAll('.accordion__item');
Array.from(itemAccord).forEach(function (item) {
  btn = item.querySelector('button');
  btn.addEventListener('click', function (e) {
    item.style.setProperty('--accordion-time', "300/1000s");
    item.classList.toggle('is-open');

    if (item.classList.contains('is-open')) {
      control = item.querySelector('.accordion__btn');
      ;
      content = item.querySelector('.accordion__body');
      ;
...
var btnsWorks = document.getElementsByClassName('works-filter__button');
Array.from(btnsWorks).forEach(function (btn) {
  btn.addEventListener("click", function (e) {
    e.preventDefault();

    if (this.querySelector('input').id === 'all') {
      Array.from(btnsWorks).forEach(function (btn) {
        btn.classList.remove('works-filter__button_pressed');
        btn.querySelector('input').checked = false;
      });
      this.classList.add('works-filter__button_pressed');
      this.querySelector('input').checked = true;
    } else {
      btnAll = document.getElementById('works-btn_first'); // btnAll = document.getElementsByClassName('works-filter__button')[0];

      btnAll.classList.remove('works-filter__button_pressed');
      btnAll.querySelector('input').checked = false;
      this.classList.toggle('works-filter__button_pressed');
      this.querySelector('input').checked ^= true;

      if (!document.querySelector('.works-filter__button_pressed')) {
        btnAll.querySelector('input').checked = true;
        btnAll.classList.add('works-filter__button_pressed');
      }
    }
  });
});
var btnsNews = document.getElementsByClassName('news-filter__button');
Array.from(btnsNews).forEach(function (btn) {
  btn.addEventListener("click", function (e) {
    e.preventDefault();

    if (this.querySelector('input').id == 'allNews') {
      Array.from(btnsNews).forEach(function (btn) {
        btn.classList.remove('news-filter__button_pressed');
        btn.querySelector('input').checked = false;
      });
      this.classList.add('news-filter__button_pressed');
      this.querySelector('input').checked = true;
    } else {
      btnAll = document.getElementById('news-btn_first'); // btnAll = document.getElementsByClassName('news-filter__button')[0];

      btnAll.classList.remove('news-filter__button_pressed');
      btnAll.querySelector('input').checked = false;
      this.classList.toggle('news-filter__button_pressed');
      this.querySelector('input').checked ^= true;

      if (!document.querySelector('.news-filter__button_pressed')) {
        btnAll.querySelector('input').checked = true;
        btnAll.classList.add('news-filter__button_pressed');
      }
    }
  });
});
...
ymaps.ready(function () {
  ...
  var placemark = new ymaps.Placemark([56.107398, 40.346762], {}, {}); // map.geoObjects.add(placemark);

  if (map) {
    ymaps.modules.require(['Placemark', 'Circle'], function (Placemark, Circle) {
      var placemark = new Placemark([56.107398, 40.346762]);
...
var circles = document.querySelectorAll('.facts-item__circle');
circles.forEach(function (el) {
  if (el.dataset.percentage == 'true') {
    var progress = el.querySelector('.progress');
    var valueBlock = el.querySelector('.facts-item__value');
    ...
    var _valueBlock = el.querySelector('.facts-item__value');
...
var worksSlider = new Swiper('.section-works__slider', {
  // Optional parameters
  slidesPerView: 1,
  spaceBetween: 30,
  loop: true,
  ...
  // Navigation arrows
  navigation: {
    nextEl: '.section-works__next',
    prevEl: '.section-works__prev'
  }
});
...
if (videoBlock) {
  ...
  var playButton = document.querySelector('.video-block__btn_play');
  playButton.addEventListener("click", function () {
    videoBlock.classList.add('video_played');
    playButton.classList.add('btn-play_played');
    video.controls = true;
    video.play();
  });

  video.onpause = function () {
    videoBlock.classList.remove('video_played');
    video.controls = false;
    playButton.classList.remove('btn-play_played');
  };
}
